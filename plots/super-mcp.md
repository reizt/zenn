...
タイトルの「任意のAPI」は、OpenAPIで記述できるHTTP APIを指す。
-->
...
この役割を担うMCPサーバーを、この記事では**メタMCP**と呼ぶ。
...
構成図はdraw.ioで作成し、`images/meta-mcp/architecture.png`へ書き出す。

### 対象範囲

タイトルの「任意のAPI」は、次の条件を満たすAPIを指す。
...
技術スタックの網羅的な紹介は避け、メタMCPの動作に関係する構成だけを書く。
...
- メタMCPは専用MCPサーバーを置き換える一般解ではなく、自作APIを短い周期で追加するための個人向け基盤として作った。
