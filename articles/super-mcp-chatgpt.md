...
以前、個人開発で食事記録やトレーニング履歴を取得するAPIを作っていました。これらのAPIをChatGPTから呼べるようにするには、APIごとにMCP endpointを実装し、ChatGPTの開発者モードへ追加すれば実現できます。最初の一つはこの方法で困りませんでした。
...
当時、Super MCPには食事記録APIとトレーニング履歴APIを登録していました。たとえば、ChatGPTへ次のように依頼していました。

> 直近4週間のトレーニング履歴から、ベンチプレスの重量と回数の推移を整理し、次回の目安を考えてください。

ChatGPTは`list_commands`で利用できる操作を調べ、トレーニング履歴を取得するコマンドを`call_command`から実行していました。Super MCPは保存してあるAPIキーをリクエストへ付与するため、**プロンプトへAPIキーを書く必要はありません**。

この例で集約したかったのは、一回のHTTPリクエストではなく、認証が必要な個人データをChatGPTから扱うための接続部分です。別の個人用APIを追加するときも、そのAPI専用のMCP endpointとOAuthを新たに用意せず、OpenAPIと認証情報の登録から試せます。
