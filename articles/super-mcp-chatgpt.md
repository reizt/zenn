...
本記事で紹介するSuper MCPは、自作APIをChatGPTから使うために業務外で開発している個人用基盤です。Super MCPを作った背景と全体構成、OpenAPIからMCPコマンドを生成する仕組み、二方向のOAuthを扱うための設計について紹介します。なお、タイトルにある「任意のAPI」は、OpenAPIで記述できるHTTP APIを指します。

### 想定読者

複数の自作HTTP APIをChatGPTへ追加するたびに、MCPと認証を重複して実装している開発者を想定しています。OpenAPIで共通化できる範囲と、接続や認証を集約したときに生じる制約を判断する材料を持ち帰ってもらうことが狙いです。

## 背景

個人開発で、WebページをMarkdownへ変換するAPIや、各種サービスのデータをGoogle Sheetsへ同期するAPIを作っています。これらのAPIをChatGPTから呼べるようにするには、APIごとにMCP endpointを実装し、ChatGPTの開発者モードへ追加すれば実現できます。最初の一つはこの方法で困りませんでした。
...
私の当初の構成では、自作APIとChatGPTが一対一で対応していました。整理すると、APIを一つ追加するたびに次の作業が発生していました。
...
ここでの問題は、MCPサーバーの実装自体が難しいことではありません。ChatGPTとの接続に加え、認証が必要なAPIでは接続先のtoken管理もあり、同じような実装をAPIごとに持たせていたことです。
...
このように、別のAPIを後から組み込めるMCPサーバーを**Super MCP**として実装しました。また、OpenAPIから生成する内部操作を**コマンド**と呼び、MCPクライアントへ直接公開するtoolと区別します。
...
WebコンソールでAPIのベースURL、OpenAPI、接頭辞、認証情報を登録すると、Super MCPへ接続設定が保存されます。ChatGPTから呼び出しを受けたWorkerは、登録されたOpenAPIから対象のoperationを特定し、HTTPリクエストを組み立てて接続先APIへ送ります。

Super MCPはCloudflare Workers上で動かしています。MCP endpointと管理APIにはHonoを使っています。WebコンソールはNext.jsをOpenNextで変換し、Worker Assetsから配信しています。

### 利用例：トレーニング履歴を振り返る

有用性が分かりやすい例として、私は[Hevy API](https://api.hevyapp.com/docs/)を登録しています。たとえば、ChatGPTへ次のように依頼します。

> 直近4週間のトレーニング履歴から、ベンチプレスの重量と回数の推移を整理し、次回の目安を考えてください。

ChatGPTは`list_commands`で利用できる操作を調べ、トレーニング履歴を取得するコマンドを`call_command`から実行します。Super MCPは保存してあるAPIキーをリクエストへ付与するため、プロンプトへAPIキーを書く必要はありません。

この例で集約したいのは、一回のHTTPリクエストではなく、認証が必要な個人データをChatGPTから扱うための接続部分です。別の個人用APIを追加するときも、そのAPI専用のMCP endpointとOAuthを新たに用意せず、OpenAPIと認証情報の登録から試せます。
...
OpenAPIを持たないAPIや、MCPのresource、prompt、samplingといった機能を使うサーバーは対象外です。Super MCPはMCPサーバーを任意に多段接続する仕組みではなく、OpenAPIで記述されたHTTP APIをMCPから呼び出せる形へ変換する仕組みです。

現在対応している入力はpath、query、header parameterとJSONのrequest bodyです。cookie parameter、multipart、binary、外部ファイルへの`$ref`、OpenAPI固有のparameter serializationは扱っていません。

## OpenAPIからMCPコマンドを生成する

Super MCPは、登録されたOpenAPIの各operationを内部のMCPコマンドへ変換します。たとえば、次のようなoperationがあるとします。
...
接頭辞として`page-kit`を設定すると、Super MCPはこのoperationを`page-kit__fetchPage`というコマンドへ変換します。コマンド名には`operationId`を使い、複数API間の衝突を避けるために接頭辞を付けます。入力スキーマはpath、query、header、request bodyから組み立てます。

選択した2xx responseまたはdefault responseの`application/json`にJSON Schemaがあれば、内部コマンドの出力スキーマとして保持します。ただし、現在の二tool構成ではoperationごとの`outputSchema`をMCP toolとして直接公開していません。

また、`list_commands`と`call_command`は`outputSchema`を宣言している一方で、実行結果を`structuredContent`ではなくtext contentだけで返しています。MCP仕様では`outputSchema`を宣言したtoolは適合するstructured resultを返す必要があるため、この部分は未修正の仕様不整合です。MCPのツール定義が持つ`inputSchema`や`annotations`については、[MCPのTools仕様](https://modelcontextprotocol.io/specification/2025-11-25/server/tools)を参照してください。
...
この例では、Super MCPが`POST /fetch`のJSON bodyへ`requestBody`を入れます。Webコンソールで登録した認証ヘッダーは入力スキーマから除外し、APIを呼び出す直前にSuper MCPが付与します。これにより、接続先APIのAPIキーをモデルへ渡したり、利用者がプロンプトへ認証情報を書いたりする必要がなくなります。
...
当時はコンテキスト量の削減よりも、APIを追加しても公開toolの名前と呼び出し方法を変えないことを優先しました。そこで、現在のSuper MCPが直接公開するMCPツールを次の二つに固定しました。
...
Super MCPには二つのOAuthがあります。一つはChatGPTからSuper MCPへの認証であり、もう一つはSuper MCPから接続先APIへの認証です。両者は別のauthorization codeとtokenを持つため、一つの中継処理として扱うとtokenの発行者と利用先が分かりにくくなります。そこでSuper MCPは、ChatGPTに対してはMCPリソースサーバー兼OAuth認可サーバーとして、接続先APIに対してはOAuthクライアントとして振る舞うようにしました。
...
図はChatGPTとの接続で使われたDynamic Client Registration構成を簡略化したものです。Dynamic Client RegistrationはMCPの必須要件ではなく、現行仕様では複数あるclient registration方式の一つです。
...
### 現行MCP Authorization仕様との差分

Super MCPはChatGPTから接続できていますが、執筆時点の実装はMCP Authorization仕様2025-11-25へ完全には適合していません。現在は認可requestの`resource`を任意としており、token requestでは`resource`を受け取らず、発行したaccess tokenにもaudienceを保持していません。
...
特に、登録されたURLをWorkerから取得する機能はSSRFの入口になります。現在は外向き通信の宛先を制限していないため、信頼できる自作API以外を登録しない運用にしています。公開するなら、schemeとhostのallowlist、private addressの拒否、redirectごとの再検証が必要です。
...
実際に審査へ提出して却下されたわけではないため、「Super MCPは審査を通らない」とは断定できません。しかし、現在の実装を不特定多数へ提供できる状態ではないと判断し、信頼できる自作APIだけを登録しています。

## まとめ

Super MCPによって、自作APIごとに用意していたChatGPTとの接続部分を一つにまとめられました。この仕組みが向くのは、同じ利用者が複数の自作APIを短い周期で追加する場合です。一つのAPIだけを公開する場合は、専用MCPサーバーのほうが単純です。したがって、Super MCPは専用MCPサーバーを置き換える一般解ではなく、自作APIを試すまでの重複を減らす個人用の基盤と位置付けています。
